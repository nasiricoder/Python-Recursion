...
def main():
    # List Comprehension.
    fact_list = [f"{x}! = {factorial(x)}" for x in range(10)]
...
def factorial(x: int):
    """This is sample of Recursion, that returns the factorial of a given number.

    Args:
        x (int): Takes an Integer as an argument.

    Returns:
        int | Literal[1]: Returns Literal integer. can be a list of integer or a dictionary of key-value-pair.
    """
